    ...
    def plot(self, autoscale=True, transform=None, edgecolor=None,
        ...
        if coords.ndim == 3:
            ...
            try:
                ...
                if isinstance(edgecolor, tuple) and \
                   all(isnumber(_) for _ in edgecolor):
                    edgecolor = [edgecolor]
                if isinstance(facecolor, tuple) and \
                   all(isnumber(_) for _ in facecolor):
                    facecolor = [facecolor]
                for c, ec, fc in zip_broadcast(coords, edgecolor, facecolor,
                                               iter_str=False):
                    a.add_patch(mp.Polygon(c, closed=True, edgecolor=ec,
                                           facecolor=fc, fill=fill,
                                           **keywords))
            except ImportError:
...
class LayoutGrid(Layout):
    ...
    ordering : array-like of int, optional
        ...
        unit = getattr(spacing, 'unit', '') or getattr(origin, 'unit', '') or \
               'radius' in keywords and getattr(keywords['radius'], 'unit', '')
        if unit:
        ...
        if unit:
            ...
            if 'vertex' in keywords:
                keywords['vertex'] = Quantity(keywords['vertex'], unit,
                                              copy=False)
...
class LayoutGridSquares(LayoutGrid):
    ...
    ordering : array-like of int, optional
        ...
        LayoutGrid.__init__(
            self, shape, spacing, vertex=vertex, xreflection=xreflection,
            yreflection=yreflection, angle=angle, origin=origin,
            startswith1=startswith1, **keywords)
